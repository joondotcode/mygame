...
function Room:init(player)
    ...
    Event.on('grabPot', function(objectHitbox)
    ...
    Event.on('throwPot',function(params)
        local pot = params.pot
        local distance = 4*16 -- can only go 4 tiles far
...
function Room:potCollidesWithWall(obj)
    if obj.x <= TILE_SIZE or obj.x >= TILE_SIZE *self.width or obj.y <= TILE_SIZE or obj.y >= self.height * TILE_SIZE then
        return true
    end
...
function Room:update(dt)
    ...
    for k, object in pairs(self.objects) do
        ...
        if object.type == 'pot' and object.state == 'thrown' then
            --Check if it collides onto the walls
            if self:potCollidesWithWall(object) then
                table.remove(self.objects, k)
            end
        ...
        if object.type == 'pot' and object.used then
            table.remove(self.objects, k)
        end
