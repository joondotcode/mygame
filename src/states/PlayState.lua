    ...
    for k, ball in pairs(self.balls) do 
        ...
        for k, brick in pairs(self.bricks) do
            ...
            if brick.inPlay and ball:collides(brick) then
                ...
                if math.random(150) < 30 then
                    if math.random(150) < 25 then
                        icon = 10
                    else
                        icon = math.random(9)
                    end
                    powertype = Powerup(ball.x, ball.y, icon)
                    table.insert(self.powerups, powertype)
                end
...
function PlayState:render()
    ...
    if self.key then
        love.graphics.draw(gTextures['main'], gFrames['powerup'][10],
            VIRTUAL_WIDTH -116, 3, 0, 0.6)
    end
