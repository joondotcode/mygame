...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        if not self.launchMarker.alien.collided then
            if not self.alienSplit and love.keyboard.wasPressed('space') then
                self.alienSplit = true

                local firstBody = self.launchMarker.alien.body

                for i = 1,2 do
                    local twoAlien
                    local firstXVel,firstYVel = firstBody:getLinearVelocity()

                    if i == 2 then
                        twoAlien = Alien(self.world, 'round', firstBody:getX(), 25 + firstBody:getY(), 'Player')
                        twoAlien.body:setLinearVelocity(firstXVel, firstYVel + 25)
                    else
                        twoAlien = Alien(self.world, 'round', firstBody:getX(), firstBody:getY() - 25, 'Player')
                        twoAlien.body:setLinearVelocity(firstXVel, firstYVel - 25)
                    end
                    table.insert(self.splitAliens, twoAlien)
                end
            end
        end
